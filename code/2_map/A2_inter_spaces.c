...
static int	ft_check_adjacent_cells(t_grl *grl, int row, int col, int prev_len, int next_len);
...
void	ft_check_internal_cell(t_grl *grl, int row, int col)
{
...
int	ft_is_internal_cell(t_grl *grl, int row, int col)
{
...
int	ft_is_space_surrounded(t_grl *grl, int row, int col)
{
	int	line_len;
	int	prev_len;
	int	next_len;
	...
	if (col <= 0 || col >= line_len - 1)
		return (0);
	if (!ft_check_adjacent_cells(grl, row, col, prev_len, next_len))
		return (0);
	return (1);
}

int	ft_check_adjacent_cells(t_grl *grl, int row, int col, int prev_len, int next_len)
{
	char	adj;
