...
{
	grl->engine.screen_buff.img_ptr = mlx_new_image(grl->engine.mlx, WIN_WIDTH,
			WIN_HEIGHT);
	if (!grl->engine.screen_buff.img_ptr)
		ft_ctrl_cleanUp(grl, "Error: mlx_new_image() falló", 1);
	grl->engine.screen_buff.addr = mlx_get_data_addr(grl->engine.screen_buff.img_ptr,
			&grl->engine.screen_buff.bpp, &grl->engine.screen_buff.line_len,
...
int	ft_game_loop(t_grl *grl)
{
	...
	mlx_put_image_to_window(grl->engine.mlx, grl->engine.win,
		grl->engine.screen_buff.img_ptr, 0, 0);
	return (0);
}