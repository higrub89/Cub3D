...
void ft_calc_step(t_ray *ray, t_grl *grl)
{
    ...
    {
        ray->step_x = 1;
        ray->side_dist_x = (ray->map_x + 1.0 - grl->player.pos_x) * ray->delta_dist_x;
    }
    if (ray->ray_dir_y < 0)
    {
        ray->step_y = -1;
    ...
    {
        ray->step_y = 1;
        ray->side_dist_y = (ray->map_y + 1.0 - grl->player.pos_y) * ray->delta_dist_y;
    }
}
