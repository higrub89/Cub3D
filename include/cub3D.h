#ifndef CUB3D_H
# define CUB3D_H

// --- INCLUDES GENERALES ---
# include "../libft/inc/libft.h"
# include "../minilibx-linux/mlx.h"
# include <errno.h>
# include <fcntl.h>
# include <math.h> // Vital para Raycasting
# include <stdio.h>
# include <stdlib.h>
# include <unistd.h>

// --- DEFINES DEL PARSER (ALEX) ---
# define HDR_NO 0
# define HDR_SO 1
# define HDR_WE 2
# define HDR_EA 3
# define HDR_F 4
# define HDR_C 5
# define HDR_TOTAL 6
# define HDR_ALL_MASK 0x3F

// --- DEFINES DEL MOTOR (RUBÉN) ---
# define WIN_WIDTH 1024
# define WIN_HEIGHT 768

// Keycodes (Linux / X11)
# define KEY_ESC 65307
# define KEY_W 119
# define KEY_A 97
# define KEY_S 115
# define KEY_D 100
# define KEY_LEFT 65361
# define KEY_RIGHT 65363

// --- ESTRUCTURAS ---

// 1. t_img (Rubén: Manejo de imágenes MLX)
typedef struct s_img
{
    void    *img_ptr;
    char    *addr;
    int     bpp;
    int     line_len;
    int     endian;
    int     width;
    int     height;
}           t_img;

// 2. t_keys (Rubén: Estado del teclado)
typedef struct s_keys
{
    int     w;
    int     a;
    int     s;
    int     d;
    int     left;
    int     right;
}           t_keys;

// 3. t_ray (Rubén: Matemáticas del Raycasting)
typedef struct s_ray
{
    double  camera_x;
    double  ray_dir_x;
    double  ray_dir_y;
    int     map_x;
    int     map_y;
    double  side_dist_x;
    double  side_dist_y;
    double  delta_dist_x;
    double  delta_dist_y;
    double  perp_wall_dist;
    int     step_x;
    int     step_y;
    int     hit;
    int     side;
    int     line_height;
    int     draw_start;
    int     draw_end;
    double  wall_x;
    int     tex_x;
    double  step;
    double  tex_pos;
}           t_ray;

// 4. t_engine (Rubén: MiniLibX y Texturas cargadas)
typedef struct s_engine
{
    void    *mlx;
    void    *win;
    t_img   screen_buff;
    t_img   textures[4]; // Las 4 texturas ya cargadas en memoria
}           t_engine;

// 5. t_player (Rubén: Física del jugador - Doubles)
typedef struct s_player
{
    double  pos_x;
    double  pos_y;
    double  dir_x;
    double  dir_y;
    double  plane_x;
    double  plane_y;
}           t_player;

// 6. t_assets (Alex: Rutas y colores crudos del parser)
typedef struct s_assets
{
    char    *textures_path[4]; // Rutas de archivos (.xpm)
    char    *colors[2];        // Strings de colores
    int     colors_rgb[2];     // Colores convertidos a Int Hex
    int     headers_mask;
}           t_assets;

// 7. t_grl (SUPER ESTRUCTURA UNIFICADA)
typedef struct s_grl
{
    // --- PARTE PARSER (ALEX) ---
    char        **map;       // Mapa crudo
    char        **map_block; // Mapa jugable validado
    int         map_start;
    int         map_end;
    int         map_height;
    int         map_width;
    int         **solid_map; // Mapa numérico (opcional para flood fill)
    int         player_row;  // Posición inicial (Int)
    int         player_col;  // Posición inicial (Int)
    int         player_dir;  // 0=N, 1=S, 2=E, 3=W
    t_assets    assets;      // Datos de configuración

    // --- PARTE MOTOR (RUBÉN) ---
    t_player    player;      // Física (Doubles)
    t_engine    engine;      // Gráficos
    t_keys      keys;        // Input
    double      old_time;    // Time delta
}           t_grl;

// --- PROTOTIPOS ---

// [MAIN & CONTROLLER]
void        ft_controller(t_grl *grl, const char *map_file);

// [1. PARSE - ALEX]
void        ft_ctrl_parse(t_grl *grl, const char *map_file);
void        ft_map_array(t_grl *grl, const char *map_file);
void        ft_headers(t_grl *grl);
void        ft_normalize_assets(t_grl *grl);
void        ft_prepare_map_block(t_grl *grl);

// --- Utils Parse (ALEX INTERNALS - ESTO FALTABA) ---
// B1_headers.c
int     ft_find_map_start_index(t_grl *grl, char **lines);
int     ft_find_map_end_index(t_grl *grl, char **lines, int map_start);
int     ft_is_line_empty(char *line);
int     ft_is_map_line(char *line);

// B2_find_map_end_index.c
void    ft_guard_against_map_gap(t_grl *grl, char **lines, int idx);
void    ft_process_header_line(t_grl *grl, char *line);
int     ft_extract_header(char *line, char **value);
void    ft_store_texture(t_grl *grl, int idx, char *value);
void    ft_store_color(t_grl *grl, int idx, char *value);

// Utils Generales Parse
int         ft_open_file(t_grl *grl, const char *map_file);
char        *ft_read_file_content(t_grl *grl, int fd);
void        ft_close_file(t_grl *grl, int fd);
void        ft_validate_file_extension(t_grl *grl, const char *map_file);
int         ft_count_lines(char *buffer);
char        **ft_allocate_lines_array(t_grl *grl, int line_count);
void        ft_split_buffer_lines(char *buffer, char **lines);

// D2_Flood.c
void		ft_flood_fill_from_borders(t_grl *grl, int **visited);
void		ft_check_accessible_spaces(t_grl *grl, int **visited);

// [2. MAP VALIDATION - ALEX]
void        ft_ctrl_map(t_grl *grl);
void        ft_validate_map_closure(t_grl *grl);
void		ft_validate_internal_spaces(t_grl *grl);
void        ft_validate_player_start(t_grl *grl);
void        ft_validate_map_characters(t_grl *grl);
void        ft_validate_exterior_access(t_grl *grl);
void        ft_normalize_map_dimensions(t_grl *grl);
void        ft_build_navigable_map(t_grl *grl);
void        ft_store_player_start(t_grl *grl);
void        ft_calculate_final_dimensions(t_grl *grl);

// [3. ENGINE - RUBÉN]
void        ft_init_motor(t_grl *grl);
void        ft_load_textures(t_grl *grl); // Carga real desde assets
void        ft_setup_hooks(t_grl *grl);
int         ft_game_loop(t_grl *grl);

// Engine Internal
void        ft_init_screen_buffer(t_grl *grl);
void        ft_render_floor_ceiling(t_grl *grl);
void        ft_raycast_walls(t_grl *grl);
void        ft_move_player_pro(t_grl *grl, double move_speed);
void        ft_rotate_player_pro(t_grl *grl, double rot_speed);
double      get_delta_time(t_grl *grl);
int         ft_rgb_to_hex(int r, int g, int b);

// [4. INTEGRATION - BRIDGE]
void        ft_convert_data(t_grl *grl); // <--- El Puente Mágico

// [CLEANUP]
void        ft_ctrl_cleanUp(t_grl *grl, char *sterror, int exit_code);
void        ft_print_error(char *sterror);
void        ft_free(t_grl *grl);
void        ft_free_array(char **array);

// [TEST/DEBUG]
void        ft_test_parser(t_grl *grl);
void        imprimir_vector(char **v);

#endif